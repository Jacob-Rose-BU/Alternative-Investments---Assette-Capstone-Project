...
3. **ESG Metrics** (embedded in the performance table): ESG pillar scores (Environmental, Social, Governance), total ESG score, and highest controversy rating are pulled (if available) and stored alongside price history as a separate `data_type = "esg"` entry for each symbol. (https://connect.sustainalytics.com/hubfs/INV/Methodology/Sustainalytics_ESG%20Ratings_Methodology%20Abstract.pdf)
