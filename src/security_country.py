...
"""
"""
CO₂ Emissions per Capita Ingestion and Processing

Purpose:
--------
This module retrieves the latest CO₂ emissions per capita data (metric tons) for all countries 
and integrates it into the ESG & Country Data portion of the pipeline. The data source is the 
World Bank, and the indicator used is `EN.ATM.CO2E.PC`.

Functionality:
--------------
1. Fetches the most recent year’s CO₂ emissions per capita from the World Bank API 
   (or CSV fallback if the API endpoint is unavailable).
2. Renames and formats columns for consistency with the rest of the data pipeline:
       - 'countryiso3code' → 'Country Code'
       - 'country' → 'Country Name'
       - 'value' → 'CO₂ Emissions per Capita (metric tons)'
3. Optionally filters to only valid countries (removes regional aggregates) 
   using a separate World Bank country metadata query.
4. Outputs a cleaned DataFrame ready for downstream merge into the Security Master 
   or ESG aggregation tables.
5. Designed to support a fallback ESG logic: if company-level ESG data is unavailable,
   the country-level CO₂ value can be normalized (0–100) and applied as part of 
   a composite ESG score.

Key Variables:
--------------
- indicator_code: World Bank indicator ID for CO₂ emissions per capita.
- label: Human-readable column name for the value field.
- co2_df: Final pandas DataFrame containing cleaned and filtered CO₂ emissions data.

Dependencies:
-------------
- pandas
- requests
- Internet access to World Bank API or CSV source

Notes:
------
If the World Bank JSON API is unavailable or the indicator is retired, 
a CSV download method should be used as a fallback to ensure uninterrupted ingestion.
